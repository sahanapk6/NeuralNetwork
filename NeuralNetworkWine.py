    ...
    def check_performance(self, y_true, y_pred):
        ...
        for i in range(len(y_true)):
            if y_true[i] == 1 and y_pred[i] == 1:
                true_positives += 1
            elif y_true[i] in [2, 3] and y_pred[i] == 1:
                false_positives += 1
            elif y_true[i] == 1 and y_pred[i] in [2, 3]:
                false_negatives += 1
            else:
                true_negatives += 1
...
def train_multiple_model(X, Y):
    ...
    hidden_layers_list = [[4], [16], [2,4],[4,8],[8,16], [16,32], [2,4,8],[4,8,16],[8,16,32], [2,4,8,16],[4,8,8,16],[4,8,16,16], [2,4,8,8,16]]
    ...
    for hidden_layers in hidden_layers_list:
        for i in range(k):
            ...
            network = NeuralNetwork(hidden_layers, 13, 3, 1, 0.5, 500)
...
def train_model(X, Y):
    ...
    network = NeuralNetwork([2,8],13, 3, 1, 0.25, 400)
    ...
    plt.ylabel('Cost')
    plt.title('Wine Dataset : Cost vs Number of iterations')
    plt.show()
